...
class WaterFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        setupRecycler()
        binding.fabAddTask.setOnClickListener { showAddWaterSheet() }
        binding.btnSetTarget.setOnClickListener { showSetTargetDialog() }
        observeData()
    }

    private fun observeData() {
        viewModel.entries.observe(viewLifecycleOwner) { rebuildList(); updateUI() }
        viewModel.dailyTarget.observe(viewLifecycleOwner) { updateUI() }
    }
    ...
    private fun showAddWaterSheet() {
        ...
        b.btnAddWater.setOnClickListener {
            ...
            if (amount == null || amount <= 0) { b.etWaterAmount.error = getString(R.string.invalid_amount); return@setOnClickListener }
            viewModel.addWater(amount) { overviewViewModel.notifyWaterChanged() }
            dialog.dismiss()
        }
    ...
    private fun showSetTargetDialog() {
        ...
        if (currentTarget > 0) b.etTarget.setText(currentTarget.toString()) else b.etTarget.text = null

        b.btnConfirmTarget.setOnClickListener {
            val target = b.etTarget.text.toString().toIntOrNull()
            if (target == null || target <= 0) { b.etTarget.error = getString(R.string.invalid_target); return@setOnClickListener }
    ...
    private fun updateUI() {
        ...
        if (target == 0) {
            ...
            ObjectAnimator.ofInt(binding.progressWater, "progress", binding.progressWater.progress, 0).apply {
                duration = 500; interpolator = AccelerateDecelerateInterpolator(); start()
            }
            return
        }

        val progress = ((consumed.toFloat() / target) * 100).toInt().coerceIn(0, 100)
        ...
        if (binding.progressWater.progress != progress) {
            ObjectAnimator.ofInt(binding.progressWater, "progress", binding.progressWater.progress, progress).apply {
                duration = 800; interpolator = AccelerateDecelerateInterpolator(); start()
            }
        }
    }
